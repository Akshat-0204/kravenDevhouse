...
export default function IntelligentSystems() {
  const [ctaMode, setCtaMode] = useState<'build' | 'evolve'>('build')
  const [hoveredStep, setHoveredStep] = useState<number | null>(null)
  const ctaLabel = useMemo(
    () => (ctaMode === 'build' ? 'Build your intelligence layer. ' : 'Think with one system. '),
    [ctaMode],
  )
  ...
  useEffect(() => {
    const timer = window.setInterval(() => {
      setCtaMode((current) => (current === 'build' ? 'evolve' : 'build'))
    }, 2200)
    ...
    <div className="bg-black text-white">
      ...
      <main className="pt-24 md:pt-28">
        <section className="relative mx-auto flex min-h-[78vh] max-w-6xl flex-col items-center justify-center overflow-hidden px-6 text-center md:px-10">
          ...
          >
         AI agents answer questions. Reporting systems surface the signals that matter. Business intelligence reveals what happens next. We build with one goal: turning scattered information into confident decisions.
...
Every intelligence system begins with questions and ends with clarity. Hover through the process to see how we transform scattered information into actionable insight.
              </p>
            </motion.div>
        ...
        <SectionBlock className="px-6 py-24 md:px-10 md:py-32" delay={0.07}>
          <div className="mx-auto max-w-4xl text-center">
            ...
            >
             Businesses are drowning in information and starving for insight. Questions pile up, reports arrive too late, and critical signals hide in plain sight. We build intelligence systems that surface answers, expose patterns, and help teams make decisions with confidence, not intuition.
        ...
        <SectionBlock className="relative px-6 py-24 md:px-10 md:py-32" delay={0.09}>
          ...
          >
            ...
            Let's give your business a second brain — one that never sleeps, forgets, or guesses.
